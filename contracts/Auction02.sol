...
contract Auction02 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    //记录每个token对应的token信息
    ...
    {
        AuctionInfo storage tokenData = tokenMap[tokenId];
        return (
            tokenData._contractAddress,
            tokenData._highestPrice,
            uint256(tokenData._currencyType)
        );
    }
}
